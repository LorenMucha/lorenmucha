...
import { isString, isObject, isBoolean, isRegExp, isFunction, isArray } from '@intlify/shared';
import { resolvePath, resolveFiles, extendPages, addWebpackPlugin, extendWebpackConfig, addVitePlugin, extendViteConfig, defineNuxtModule, isNuxt2, getNuxtVersion, isNuxt3, addPlugin, addTemplate, addImports } from '@nuxt/kit';
import { dirname, resolve, parse, normalize, relative } from 'pathe';
...
async function getPackageManagerType() {
  try {
    ...
    debug$7("getPackageManagerType: lockfile", lockfile);
    if (lockfile == null) {
      return "unknown";
    }
    ...
    debug$7("getPackageManagerType: resolveLockfile error", e);
    throw e;
  }
}
async function resolveVueI18nPkgPath() {
  const p = await resolvePath("vue-i18n");
  debug$7("vue-i18n resolved path", p);
  return resolve(p, "../..");
}
async function resolveVueI18nRoutingDtsPath(id) {
  const pkgMgr = await getPackageManagerType();
  if (pkgMgr === "npm") {
    debug$7("resolveVueI18nRoutingDtsPath on npm", `${VUE_I18N_ROUTING_PKG}/dist/${id}`);
    return resolve(pkgModulesDir, `${VUE_I18N_ROUTING_PKG}/dist/${id}`);
  } else if (pkgMgr === "pnpm" || pkgMgr === "yarn") {
    const parsed = parse(await resolvePath(VUE_I18N_ROUTING_PKG));
    debug$7(`resolveVueI18nRoutingDtsPath on ${pkgMgr}`, parsed);
...
async function resolveVueI18nBridgeAlias(pkgModuleDir, pkgMgr) {
  if (pkgMgr === "npm") {
    ...
    const parsed = parse(await resolvePath(VUE_I18N_BRIDGE_PKG));
    debug$6(`resolveVueI18nBridgeAlias on ${pkgMgr}`, parsed);
    return `${parsed.dir}/${parsed.name}.mjs`;
  }
}
async function resolveVueRouterBridgeAlias(pkgModuleDir, pkgMgr) {
  if (pkgMgr === "npm") {
    debug$6("resolveVueRouterBridgeAlias on npm", `${VUE_ROUTER_BRIDGE_PKG}/lib/index.mjs`);
    return resolve(pkgModuleDir, `${VUE_ROUTER_BRIDGE_PKG}/lib/index.mjs`);
  } else {
    const parsed = parse(await resolvePath(VUE_ROUTER_BRIDGE_PKG));
    debug$6(`resolveVueRouterBridgeAlias on ${pkgMgr}`, parsed);
    return `${parsed.dir}/${parsed.name}.mjs`;
  }
}
async function resolveVueI18nRoutingAlias(pkgModuleDir, pkgMgr) {
  if (pkgMgr === "npm") {
    debug$6("resolveVueI18nRoutingAlias on npm", `${VUE_I18N_ROUTING_PKG}/dist/vue-i18n-routing.mjs`);
    return resolve(pkgModuleDir, `${VUE_I18N_ROUTING_PKG}/dist/vue-i18n-routing.mjs`);
  } else if (pkgMgr === "pnpm" || pkgMgr === "yarn") {
    const parsed = parse(await resolvePath(VUE_I18N_ROUTING_PKG));
    debug$6(`resolveVueI18nRoutingAlias on ${pkgMgr}`, parsed);
...
async function resolveLocales(path, locales) {
  ...
  return files.map((file) => {
    ...
    const locale = locales.find((locale2) => isObject(locale2) && locale2.file === parsed.base);
    return locales == null ? {
      path: file,
      file: parsed.base,
      code: parsed.name
...
async function extendMessages(nuxt, localeCodes) {
...
async function normalizeAdditionalMessages(additional, localeCodes) {
  const additionalMessages = {};
...
function generateLoaderOptions(lazy, langDir, localesRelativeBase, options = {}, dev = true) {
  let genCode = "";
...
`;
      for (const [key, value] of Object.entries(rootValue)) {
        if (key === "vueI18n") {
          ...
          genCodes += `  const ${optionLoaderVariable} = ${isObject(value) ? `async (context) => ${generateVueI18nOptions(value, dev)}
...
const module = defineNuxtModule({
  ...
  async setup(i18nOptions, nuxt) {
    ...
    const isLegacyMode = () => {
      return isString(options.types) ? options.types === "legacy" : isObject(options.vueI18n) ? options.vueI18n.legacy : false;
    };
